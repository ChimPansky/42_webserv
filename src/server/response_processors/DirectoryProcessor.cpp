...
bool DirectoryProcessor::ListDirectory_(const std::string& path) {
    ...
    for (size_t i = 0; i < dir_entries.second.size(); i++) {
        body_stream << "<li><a href=\"" << path << "/" << dir_entries.second[i].name() << "\">" << dir_entries.second[i].name() << "</a></li>\n";
    }
    body_stream << "</ul>\n"
                "</body>\n"
                "</html>\n";
    ...
    // todo: instead of creating the whole response at once, stream the body to the client with chunks...
    response_rdy_cb_->Call(utils::unique_ptr<http::Response> (new http::Response(http::HTTP_OK, http::HTTP_1_1, hdrs, body)));
    return true;
}


std::pair<bool /*success*/, std::vector<DirectoryProcessor::DirEntry> /*dir_entries*/> DirectoryProcessor::GetDirEntries_(const char* directory) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        entries.push_back(DirectoryProcessor::DirEntry(entry->d_name, std::string("<path_fo_file>"), DE_FILE, false));
    }
    closedir(dir);
