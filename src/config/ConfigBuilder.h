...
class ConfigBuilder<LocationConfig> {
    ...
    private:
        ...
        static const std::string&    BuildRoute(const std::vector<std::string>& vals)
        {
            ...
            std::vector<std::string>    route_elements = config::SplitLine(vals[0]);
            return ParseRoute(route_elements);
        }

        static const std::string&    ParseRoute(const std::vector<std::string>& vals)
        {
            if (vals.size() > 2) {
            ...
            if (vals[0] == "=" || vals[0] == "^~")
            {
                ...
                throw std::runtime_error("Invalid configuration file: invalid route syntax: " + vals[0]);
            }
            return vals[1];
        }
        ...
        static std::vector<std::string> BuildAllowedMethods(const std::vector<std::string>& vals)
        {
            ...
            return ParseAllowedMethods(vals);
        }
        ...
        static std::pair<int, std::string> ParseRedirect(const std::vector<std::string>& vals)
        {
            ...
            int code = std::stoi(vals[0]);
            if (access(vals[1].c_str(), F_OK | R_OK | W_OK) == -1) {
                throw std::runtime_error("Invalid configuration file: index file doesn't exist: " + vals[1]);
            }
            return std::make_pair(code, vals[1]);
        }

        static const std::vector<std::string>& BuildCgiPaths(const std::vector<std::string>& vals)
        {
            if (vals.empty()) {
                throw std::runtime_error("Invalid configuration file: no cgi_path specified.");
            }
        ...
        static const std::vector<std::string>& ParseCgiPaths(const std::vector<std::string>& vals)
        {
            for (size_t i = 0; i < vals.size(); i++) {
                if (access(vals[i].c_str(), F_OK | R_OK) == -1) {
                    throw std::runtime_error("Invalid configuration file: index file doesn't exist: " + vals[i]); // default?
                }
            }
            return vals;
        }

        static const std::vector<std::string>& BuildCgiExtensions(const std::vector<std::string>& vals)
        {
            if (vals.empty()) {
                throw std::runtime_error("Invalid configuration file: no cgi_extension specified."); // or default?
            }
            std::vector<std::string>    val_elements = config::SplitLine(vals[0]);
        ...
        static const std::vector<std::string>& ParseCgiExtensions(const std::vector<std::string>& vals)
        {
            for (size_t i = 0; i < vals.size(); i++) {
                if (vals[i] == ".py" || vals[i] != ".php") {
                    throw std::runtime_error("Invalid configuration file: invalid cgi_extension: " + vals[i]);
                }
            }
            return vals;
        }
        ...
        static const std::string& ParseDefaultFile(const std::string& vals)
        {
            if (access(vals.c_str(), F_OK | R_OK | W_OK) == -1) {
                throw std::runtime_error("Invalid configuration file: index file doesn't exist: " + vals);
            }
            return vals;
        }
        ...
        static bool IsKeyAllowed(const std::string& key) {
            return key == "allow_methods" || key == "return" || key == "cgi_path" || key == "cgi_extension" 
    ...
    public:
        static LocationConfig Build(const ConfigParser& f) {
                ...
                std::string route = BuildRoute(f.FindSetting("route"));
                std::vector<std::string> allowed_methods = BuildAllowedMethods(f.FindSetting("allow_methods"));
                ...
                return LocationConfig(route, allowed_methods, redirect, cgi_paths, cgi_extensions, root_dir, default_file, dir_listing);
        }
...
class ConfigBuilder<ServerConfig> {
    ...
    private:
        ...
        static std::vector<std::pair<in_addr_t, in_port_t> >   BuildListeners(const std::vector<std::string>& vals)
        {
            if (vals.empty()) {
        ...
        static std::vector<std::string>    ParseServerNames(const std::string& vals)
        {
            ...
            for (size_t i = 0; i < server_names.size(); i++) {
                if (server_names[i].empty()) {
                    throw std::runtime_error("Invalid configuration file: invalid server_name: " + vals);
                }
        ...
        static std::vector<LocationConfig>  BuildLocationConfigs(const ConfigParser& f)
        {
            ...
            for (size_t i = 0; i < f.nested_configs().size(); i++) {
                server_configs.push_back(ConfigBuilder<LocationConfig>::Build(f.FindNesting("location", i)));
            }
        ...
        static bool IsKeyAllowed(const std::string& key) {
            return key == "listen" || key == "server_name" || key == "access_log" || key == "error_log"
                    || key == "root" || key == "index" || key == "autoindex" || key == "location";
        }
    ...
    public:
            static ServerConfig Build(const ConfigParser& f) {
                ...
                std::vector<std::pair<in_addr_t, in_port_t> > listeners = BuildListeners(f.FindSetting("listen"));
                ...
                return ServerConfig(access_log_path, access_log_level, error_log_path, listeners, root_dir, default_file, dir_listing, server_names, location_configs);
            }
};
...
class ConfigBuilder<HttpConfig> {
    ...
    static std::vector<ServerConfig>  BuildServerConfigs(const ConfigParser& f)
    {
        ...
        for (size_t i = 0; i < f.nested_configs().size(); i++) {
            server_configs.push_back(ConfigBuilder<ServerConfig>::Build(f.FindNesting("server", i)));
        }
        return server_configs;
    }
