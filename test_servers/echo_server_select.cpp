...
int main()
{
    ...
    while (1) {
        ...
        for (set_it it = SlaveSockets.begin(); it != SlaveSockets.end(); ++it) {
        ...
        int num_of_fds = select(maxfd + 1, &select_rd_set, /* write fds */ NULL, /* err fds */ NULL,
                                /* timeout */ NULL);
        if (num_of_fds <= 0) {
        ...
        for (int i = 0; i <= maxfd; ++i) {
            set_it it;
            if (FD_ISSET(i, &select_rd_set) &&
                ((it = SlaveSockets.find(i)) != SlaveSockets.end())) {
