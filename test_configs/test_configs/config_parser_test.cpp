...
TEST(ConfigTest, LoadValidConfig)
{
    ...
    config::Config conf = config::Config::GetConfig(valid_file);
...
TEST(ConfigTest, MinimumSettingsConfig)
{
    ...
    EXPECT_EQ(http_config.keepalive_timeout(), 65);
    EXPECT_EQ(http_config.client_max_body_size(), 2ul << 20);
...
TEST(ConfigTest, ValidDuplicates)
{
    std::string valid_file = "./test_configs/valid_duplicates.conf";
    char buf[1024];
    std::cout << getcwd(buf, sizeof(buf)) << std::endl;
    config::Config conf = config::Config::GetConfig(valid_file);
    ...
    for (const config::ServerConfig& server_conf : server_configs) {
        if (server_conf.listeners()[0].second == 8080) {
            ...
            EXPECT_EQ(server_conf.error_log_path(), "");
            EXPECT_EQ(server_conf.server_names()[0], "www.example.com");
            EXPECT_EQ(server_conf.server_names()[1], "*.example.com");
...
TEST(ConfigTest, InvalidDuplicates1)
{
    std::string invalid_file = "test_configs/invalid_duplicates.conf";

    EXPECT_THROW(config::Config::GetConfig(invalid_file), std::exception);
}

TEST(ConfigTest, InvalidDuplicates2)
{
    std::string invalid_file = "test_configs/invalid_duplicates2.conf";

    EXPECT_THROW(config::Config::GetConfig(invalid_file), std::exception);
}

TEST(ConfigTest, InvalidDuplicates3)
{
    std::string invalid_file = "test_configs/invalid_duplicates3.conf";
