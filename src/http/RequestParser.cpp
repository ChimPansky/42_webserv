...
std::vector<unsigned char>& RequestParser::buf()
{
    return buf_;
}

void RequestParser::ExpandBuf(size_t recv_size)
{
    old_buf_size_ = buf_.size();
    buf_.resize(buf_.size() + recv_size);
}

void RequestParser::ShrinkBuf(size_t bytes_recvd)
{
...
{
    return std::strncmp((reinterpret_cast<const char*>(buf_.data())) + element_begin_idx(), str, len);
}
...
unsigned char& RequestParser::operator[](ssize_t index)
{
    if (index < 0) {
        return buf_[0];
    }
