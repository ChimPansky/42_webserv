        ...
        if (server_conf.listeners()[0].second == 8080) {
            ...
            for (const config::LocationConfig& location_conf : server_conf.locations()) {
                if (location_conf.route().first == "/docs/") {
                    ...
                    EXPECT_EQ(location_conf.allowed_methods()[0], "GET");
                    EXPECT_EQ(location_conf.allowed_methods()[1], "POST");
...
{
    std::string invalid_file = "test_configs/invalid_allow_methods.conf";
