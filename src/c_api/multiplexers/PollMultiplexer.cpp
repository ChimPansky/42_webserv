...
void PollMultiplexer::CheckOnce()
{
    ...
    for (std::vector<struct pollfd>::iterator it = fds.begin(); it != fds.end(); ++it) {
        if (it->revents & POLLIN) {
            FdToCallbackMapIt sockets_it = rd_sockets_.find(it->fd);
            if (sockets_it != rd_sockets_.end()) {
                sockets_it->second->Call(sockets_it->first);
            }
        }
        if (it->revents & POLLOUT) {
            FdToCallbackMapIt sockets_it = wr_sockets_.find(it->fd);
            if (sockets_it != wr_sockets_.end()) {
                sockets_it->second->Call(sockets_it->first);
            }
        }
    }
