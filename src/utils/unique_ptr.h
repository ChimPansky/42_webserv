...
/**
 * @file  unique_ptr.hpp
 * @brief unique_ptr is a fake implementation to use in place of a C++11 std::unique_ptr when
 * compiling on an older compiler.
 *
...
/**
 * @brief minimal implementation of unique pointer, a subset of the C++11 std::unique_ptr or
 * boost::unique_ptr.
 *
 ...
 *
 * This implementation is a fake one, to use when compiling on an older compiler that does not have
 * the move semantic. As such, it cannot offer any guaranty that the pointer is not copied! It is
 * still usable in production on such a compiler if you also compile your program on a recent
 * compiler to verify that it does not violate this guaranty.
 */
template <class T>
class unique_ptr {
  public:
    ...
    /* TODO MSVC error C2248: 'unique_ptr<B>::px' : unique_ptr<A> cannot access private member
    declared in class 'unique_ptr<B>' template <class U> unique_ptr(const unique_ptr<U>& ptr)
    throw() : // never throws px(static_cast<typename unique_ptr<T>::element_type*>(ptr.px))
    {
        const_cast<unique_ptr<U>&>(ptr).px = NULL; // const-cast to force ownership transfer!
    }
    */
    /// @brief Copy constructor (used by the copy-and-swap idiom)
    unique_ptr(const unique_ptr& ptr) throw()
        :  // never throws
          px(ptr.px)
    {
  ...
  private:
    ...
    /// @brief hack: const-cast release the ownership of the px pointer without destroying the
    /// object!
