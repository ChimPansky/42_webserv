...
utils::shared_ptr<IMultiplexer> GetMultiplexer(MultiplexType mx_type)
{
    switch (mx_type) {
        case MT_SELECT:
            return utils::shared_ptr<IMultiplexer>(new SelectMultiplexer());
        case MT_EPOLL:
            return utils::shared_ptr<IMultiplexer>(new EpollMultiplexer());
        default:
            LOG(FATAL) << "unknown mx_type";
            break;
    }
    return utils::shared_ptr<IMultiplexer>(
