...
{
    LOG(INFO) << "";
    LOG(INFO) << "ClientSocket::Recv(std::vector<char>& buf, read_size: " << read_size << "):";
    LOG(INFO) << "  this makes sure that buffer is large enough by resizing it";
    LOG(INFO) << "  and then reads directly into it";
    LOG(INFO) << "  (Yes, i know you love it, Vova)";
